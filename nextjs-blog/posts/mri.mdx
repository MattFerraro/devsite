...
If one dipole is much larger and stronger than the other then near the small dipole the magnetic field of the big dipole looks uniform and we can simplify our model a bit.

Here I've replaced the larger dipole with a uniform background field. Once again the dominant behavior is to align.
...
Notice that if you make the external field stronger, the alignment torque gets stronger and the oscillation frequency gets faster.
...
Protons, electrons, and neutrons all exhibit **magnetic moment** which means they all behave like tiny little bar magnets.

<!-- Both protons and neutrons exhibit spin and are composed of smaller particles called Quarks which carry electric charge. Spinning electric charges create dipoles so all protons and all neutrons behave like tiny little bar magnets. -->

A Hydrogen atom, $^{1}_{0}\text{H}$, contains a single proton and a single electron. When exposed to an external magnetic field both the proton and the electron will feel a torque that tries to align them with that field exactly like tiny compass needles.
