...
It's time to build a new open-source CAD program. We've gotten pretty far, but we need your help.
...
Our goal is to create a CAD application where every single document state is trivially recoverable. Every false start, every "final" deliverable, whether you knew you needed it or not.
