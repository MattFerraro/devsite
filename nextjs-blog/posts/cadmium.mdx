---
title: "CADmium"
teaser: "It's time to build a new open-source CAD program"
teaserImage: "https://www.mattferraro.dev/images/ci-for-cad/pipes.png"
date: "2024-03-11"
---

It's time to build a new open-source CAD program. I've gotten pretty far, but I need your help.
...
To build a 3D parametric CAD program, you need five major components:

1. A 2D Constraint Solver
2. A B-rep Kernel
3. A History Tracker
4. A 3D User Interface
5. A File Format
...
There are other interesting formulations of the 2D constraint problem and I'm sure there are disadvantages to the spring-mass-damper approach, but in general I would advocate for solving the problem iteratively rather than in a single monolithic solve step. In the last decade there has been tremendous progress in solving gradient descent problems quickly, and in bringing the power of the GPU to the browser.

My primary goal is to make a CAD experience that feels familiar for most CAD users, but I do believe there is room here for fresh ideas.

# B-rep Kernel

In Mechanical CAD, users need to interact directly with the edges and faces of their parts.

Consequently, all modern CAD programs model parts by directly representing their boundaries in a data structure. A cube would be representated as a Solid which has six Faces, each with four Edges, each with two Points. This approach is called _Boundary Representation_, or _b-rep_.
...
Most notably, Parasolid powers a lot of the industry including other products like Shapr3D and Plasticity. Parsolid is the Cadillac Escalade of b-rep kernels: It is huge, expensive, and it offers every amenity you could ask for, as well as a bunch of amenities you didn't ask for.
...
SolveSpace is a [Tuk-Tuk](https://en.wikipedia.org/wiki/File:Tuk-Tuk_-_Herat,_Afghanistan.jpg) in that it was built by one person in a garage, it gets a lot done with very little, but it only looks like a car if you squint.
...
Which appears to have been taken on a Windows 2000 machine.
...
It is small and lightweight, it is sponsored by a [real company](https://www.ricos.ltd/), and it is young enough and simple enough that a few motivated people could add major pieces of functionality, in a fork if necessary.

For example, the B in NURBS stands for [B-Splines](https://en.wikipedia.org/wiki/B-spline), but there is an alternative representation called [T-Splines](https://en.wikipedia.org/wiki/T-spline) which is better in some ways. The [patent](https://patents.google.com/patent/US7274364B2/en) on T-Splines is owned by Autodesk, but it just expired last week! Support could theoretically be built into Truck!
...
Which is a good segue to a third idea:

Why not record every user event in an append-only log? If that log were the single source of truth for the file, then any particular Feature History could be reconstructed by moving a "time" slider. Think: Unlimited Undo/Redo, even after closing and reopening the file:
...
My goal is to create a CAD application where every single document state is trivially recoverable. Every false start, every "final" deliverable, whether you knew you needed it or not.
...
If building this system is akin to building git version control for Mechanical design, what would it look like to build git**hub** for Mechanical design?

# 3D User Interface

I love the idea of doing CAD in a browser. Onshape paved the way here and it's awesome.
...
But CADmium doesn't have to be like that. Given that Truck can compile to webassembly, CADmium can do everything right there in your browser.

---

I've been using this tech stack:

1. [Three.js](https://threejs.org/) for the 3D viewport
1. [Svelte](https://svelte.dev/) for state management/reactivity
1. [Threlte](https://threlte.xyz/) to bridge the gap between Svelte and Three.js
1. Message passing between the UI and the b-rep kernel, rather than sharing memory
1. [Electron](https://www.electronjs.org/) for running locally
1. Bog standard everything else: Typescript, TailwindCSS, Vite, etc

This kind of stack allows the entire app to be written in a reactive, declarative way, plumbing data changes all the way through to mesh updates without you having manage that complexity. That's important because 3D CAD apps are among the most complex UIs that exist. There's no way for a small team to make a good one unless the framework does a lot of heavy lifting.

With this stack I was able to build a proof of concept that works, so I feel that this won't be the limiting factor for CADmium.

# File Format

I want CADmium to use JSON or [JSON-adjacent](https://jsonlines.org/) files for everything, especially the Operation Log mentioned above.

And after you've designed a part, CADmium should support exporting to an even simpler exchange format. Notionally something like:
...
Are what's required to enable an ecosystem that can create tremendous new value that I'd never be able to build myself.
...
I don't know exactly what that would need to look like in practice, but I think it's fair to say that large language models work best on simple, open, text-based formats rather than complex, proprietary, binary formats.

# Conclusion

Of the ideas I mentioned here, I have no idea which ones are going to work out and which ones will turn out to be duds. But I know that somewhere in this space, there's a huge opportunity for a small group of people to make an outsized impact on the manufacturing industry.

These are the things I actively need help with:
...
These are things that we are not touching for now:

1. Venture funding
1. Toolpath generation (CAM)
1. Finite Element Analysis (FEA)

If you find these ideas intriguing, please join the CADmium [discord server](https://discord.gg/qJCsKJeyZv) and chat with me!

---

## Delete everything below this line

Company: I'd rather build a company like Obsidian than a company like Notion.

great blog post by Shapr3D: https://www.shapr3d.com/blog/what-is-cad-the-technological-foundations-of-cad-software

for rendering huge assemblies: slow problems are

1. solving all the constraints for all the moving parts
2. rendering huge meshes with many, many triangles

Part 2: the nitty gritty

1. The topological naming problem and our resolution: anchors. abstraction on its own sucks. you only create a layer of abstraction if it gives the user a way to be extremely specific in some way that they weren't able to be before.
