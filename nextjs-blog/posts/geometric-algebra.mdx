...
On a lark, what if we borrow an idea from CAD software and try to sweep or *extrude* $\vec{a}$ in the direction of $\vec{b}$?
...
These three unit bivectors are orthogonal to each other, which just means that if you project the yz plane onto the xy plane, its shadow occupies zero area. This is true for any pair of these planes.
