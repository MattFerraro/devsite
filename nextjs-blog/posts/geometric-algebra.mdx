...
Which, when broken down into its scalar, vector, bivector, and trivector components, correspond to [Maxwell's four equations](https://en.wikipedia.org/wiki/Maxwell%27s_equations)! All of electricity and magnetism in one single equation!
...
In stumbling toward an answer, we developed a much richer understanding of what a vector is and what it does. Along the way we derived the foundation of a branch of Math that is usually only taught to graduate students, but is actually suitable for high school students.

We built a set of algebraic tools so powerful and elegant that they make the dominant system of mathematical notation in science and engineering seem a little silly.
...
In this post I took several liberties with notation and naming. What I called the "extrude product" is usually called the "wedge product" or "exterior product". Bivectors are often written like $\vec{B}$ rather than $\overset{\Rightarrow}{b}$. Most CAD software uses the word "sweeping" to create a 2D surface, and "extrusion" which creates a 3D surface.
