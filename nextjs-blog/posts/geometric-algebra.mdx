...
For the purposes of this post a **vector** $\vec{a}$ is an arrow in 3D space. It points from the origin to some other point in space and is usually written like this:
...
$$
\vec{a} = a_1\mathbf{\hat{x}} + a_2\mathbf{\hat{y}} + a_3\mathbf{\hat{z}}
$$

![A Typical Vector](/images/geometric-algebra/vector.svg)
...
![Our First Attempt at a Bivector](/images/geometric-algebra/bivector_first_try.svg)
...
![Bivector Second Try](/images/geometric-algebra/bivector_second_try.svg)

Which has an area equal to:

$$
A_{triangle}=\frac{1}{2}\|\textcolor{red}{\vec{a}}\|\textcolor{blue}{\vec{b}}\|\sin(\theta)
$$
...
On a lark, what if we borrow an idea from CAD software and try to sweep or *extrude* $\textcolor{red}{\vec{a}}$ in the direction of $\textcolor{blue}{\vec{b}}$?

![Bivector Second Try](/images/geometric-algebra/bivector_third_try.svg)
...
$$
A_{parallelogram}=\|\textcolor{red}{\vec{a}}\|\|\textcolor{blue}{\vec{b}}\|\sin(\theta)
$$
...
![Doubling a Bivector](/images/geometric-algebra/bivector_doubling.svg)
...
![Reversing a Bivector](/images/geometric-algebra/bivector_reversing.svg)

Well, $\vec{a} \wedge \vec{b}$ forms a red parallelogram but $\vec{b} \wedge \vec{a}$ forms a blue parallelogram. Again we get to choose if these should be considered equal.
...
Your complaint is valid, and plus we don't always get to use color when we're sketching in a notebook so let's try looking at this situation a way that doesn't rely on color:

![Reversing a Bivector View 2](/images/geometric-algebra/bivector_reversing_2.svg)

We can see now that $\vec{a} \wedge \vec{b}$ forms a parallelogram by spinning clockwise, but $\vec{b} \wedge \vec{a}$ forms a parallelogram by spinning anti-clockwise.
...
Note that we're not necessarily saying that clockwise spin is positive and counterclockwise is negative, we're just saying that whatever the two spin directions are, they're opposites.

What happens if you extrude a vector by itself?

![Extruding a vector by itself](/images/geometric-algebra/vector_extrude_itself.svg)
...
![x extrude y](/images/geometric-algebra/x_extrude_y.svg)

The parallelogram they form happens to be a square with unit area! We can use this to form three distinct unit bivectors:

- $\mathbf{\hat{x}}\wedge\mathbf{\hat{y}}$
- $\mathbf{\hat{y}}\wedge\mathbf{\hat{z}}$
- $\mathbf{\hat{z}}\wedge\mathbf{\hat{x}}$

![The Unit Bivectors](/images/geometric-algebra/unit_bivectors.svg)

The ordering of the unit vectors doesn't really matter as long as you pick one thing and stick to it, but we should pick something that is easy to remember. I think we should use $\mathbf{\hat{x}}\wedge\mathbf{\hat{z}}$ instead of $\mathbf{\hat{z}}\wedge\mathbf{\hat{x}}$ because that way, every unit bivector is just spelled out alphabetically. This ruins our nice one-of-each-color pattern, but it makes for some easier manipulation later on.

The three unit bivectors are orthogonal to each other, which just means that if you project the yz plane onto the xy plane, its shadow occupies zero area. This holds true for any pair of these planes.
...
$$
\vec{a} \wedge \vec{b} = c_1(\mathbf{\hat{x}} \wedge \mathbf{\hat{y}}) + c_2(\mathbf{\hat{y}}\wedge\mathbf{\hat{z}}) + c_3(\mathbf{\hat{x}}\wedge\mathbf{\hat{z}})
$$

Where you find the coefficients $c_1$, $c_2$, $c_3$ by projecting the parallelogram $\vec{a} \wedge \vec{b}$ onto the unit bivectors $\mathbf{\hat{x}} \wedge \mathbf{\hat{y}}$, $\mathbf{\hat{y}}\wedge\mathbf{\hat{z}}$, $\mathbf{\hat{x}}\wedge\mathbf{\hat{z}}$.
...
![An Example Trivector](/images/geometric-algebra/example_trivector_2.svg)
