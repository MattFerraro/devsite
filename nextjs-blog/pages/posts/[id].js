...
const renderChild = (child, scene, scene2) => {
  ...
  if (childAttrs.class.value === "Arrow") {
    const end = new THREE.Vector3(
      ...childAttrs.end.value.split(",").map(parseFloat)
    )
    const begin = new THREE.Vector3(
      ...childAttrs.begin.value.split(",").map(parseFloat)
    )
    ...
    scene.add(
      new THREE.ArrowHelper(
        dir,
        begin.multiplyScalar(arbitraryScaling),
        length * arbitraryScaling,
        color,
        headLength * arbitraryScaling,
        headWidth * arbitraryScaling
      )
    )
  } else if (childAttrs.class.value === "DottedLine") {
    const end = new THREE.Vector3(
      ...childAttrs.end.value.split(",").map(parseFloat)
    ).multiplyScalar(arbitraryScaling)
    const begin = new THREE.Vector3(
      ...childAttrs.begin.value.split(",").map(parseFloat)
    ).multiplyScalar(arbitraryScaling)
    const color = childAttrs.color.value
    ...
    const material = new THREE.LineDashedMaterial({
      ...
      linewidth: 1,
      scale: 1,
      dashSize: 0.05 * arbitraryScaling,
      gapSize: 0.05 * arbitraryScaling,
    })
    ...
    if (childAttrs.root && childAttrs.root.value) {
      const root = new THREE.Vector3(
        ...childAttrs.root.value.split(",").map(parseFloat)
      ).multiplyScalar(arbitraryScaling)
...
const Vis3D = (props) => {
  return (
    ...
    >
      ...
      ></div>
      <div className="Vis3D-Elements">{props.children}</div>
    </div>
  )
}
...
const AudioControls2 = (props) => {
  return (
    <div
      ...
      src={props.src}
      stereo={props.stereo ? "true" : "false"}
    >
...
const UltimateCircleInverter = (props) => {
  return (
    <canvas
      className="ultimate-circle-inverter"
      width={614}
      height={400}
      style={{ width: 614, height: 400, cursor: "crosshair" }}
      {...props}
    ></canvas>
  )
}
...
const ComplexNumberInverter = (props) => {
  return (
    <canvas
      className="complex-number-inverter"
      width={614}
      height={400}
      style={{ width: 614, height: 400, cursor: "crosshair" }}
    ></canvas>
...
const initialize3D = (vis3DContainer) => {
  ...
  const cameraPos = new THREE.Vector3(
    ...attrs.camera.value.split(",").map(parseFloat)
  ).multiplyScalar(arbitraryScaling)
...
const CircleInverterRealizer = dynamic(
  () => import("../../components/circleinverterrealizer"),
  {
    ssr: false,
  }
)
...
const DipoleWebGL = dynamic(() => import("../../components/mri/dipolewebgl"), {
  ssr: false,
})
const Nucleons = dynamic(() => import("../../components/mri/nucleons"), {
  ssr: false,
})
          ...
          components={{
            ...
            p: (paragraph) => {
              if (
                ...
                paragraph.children.props.mdxType &&
                paragraph.children.props.mdxType === "img"
              ) {
                  ...
                  if (childProps.title && childProps.title === "dno") {
                    return (
                      <Image
                        priority={true}
                        unoptimized={true}
                        src={paragraph.children.props.src}
                        alt={paragraph.children.props.alt}
                        width={imageWidth}
