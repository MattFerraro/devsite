...
const forceColor = 0x0d6efd
const torqueColor = 0x6500bd
const radiusColor = 0x00bf19
...
const Gyroscope = () => {
    ...
    new RGBELoader()
      ...
      .load("studio_small_09_1k.hdr", (hdrmap) => {
        ...
        objLoader.load("/resources/mri/frame.obj", (root) => {
          ...
          const amArrow = new THREE.ArrowHelper(
            new Vector3(0, 0, 1),
            new Vector3(0, 0, 0),
            0.16,
            angularMomentumColor,
            0.01,
            0.007
          )
          pivot.add(amArrow)

          const torqueArrow2 = new THREE.ArrowHelper(
            new Vector3(0, 1, 0),
          ...
          const FArrow2 = new THREE.ArrowHelper(
            new Vector3(Math.sin(tiltAngle), 0, -Math.cos(tiltAngle)),
            new Vector3(0, 0, 0.065),
            0.03,
            forceColor,
            0.01,
            0.007 * gravityRef.current
          )
          pivot.add(FArrow2)

          const rArrow = new THREE.ArrowHelper(
            new Vector3(Math.cos(tiltAngle), 0, Math.sin(tiltAngle)),
            new Vector3(0, 0, 0),
            Math.sin(tiltAngle) * 0.065,
            radiusColor,
            0.01,
            0.007
          )
  ...
  const update = () => {
    if (sceneContext.current.pivot) {
      ...
      pivot.rotateOnWorldAxis(new Vector3(0, 0, 1), gravityRef.current * 0.01)

      if (sceneContext.current.spinner) {
        const spinner = sceneContext.current.spinner
        spinner.rotateOnAxis(new Vector3(0, 0, 1), 0.2)
      }

      if (sceneContext.current.FArrow) {
        sceneContext.current.FArrow.setLength(
          gravityRef.current * 0.01,
          Math.min(0.01, gravityRef.current * 0.01),
          Math.min(0.007, gravityRef.current * 0.01)
        )
      }
      if (sceneContext.current.torqueArrow) {
        sceneContext.current.torqueArrow.setLength(
          gravityRef.current * 0.01,
          Math.min(0.01, gravityRef.current * 0.01),
          Math.min(0.007, gravityRef.current * 0.01)
        )
